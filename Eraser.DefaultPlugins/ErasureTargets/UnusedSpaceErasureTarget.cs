...
namespace Eraser.DefaultPlugins
{
	...
	public class UnusedSpaceErasureTarget : ErasureTargetBase
	{
		...
		public override void Execute()
		{
			...
			var tp = Path.GetFileName(
				FileSystemBase.GenerateRandomFileName(info, 18));

            //not working in NET 8.0
            //info = info.CreateSubdirectory(tp);
            //alternative start
            if (!Directory.Exists(Path.Combine(info.FullName, tp)))
			{
                Directory.CreateDirectory(Path.Combine(info.FullName, tp));
            }
            info = new DirectoryInfo(Path.Combine(info.FullName, tp));
			//alternative end
