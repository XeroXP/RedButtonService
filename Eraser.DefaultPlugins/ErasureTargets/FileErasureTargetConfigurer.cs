...
{
	partial class FileErasureTargetConfigurer : IErasureTargetConfigurer
	{
		...
		public bool SaveTo(IErasureTarget target)
		{
			...
			if (filePathText.Length == 0)
			{
				return false;
			}

			file.Path = filePathText;
			return true;
		}
  ...
  argument: file=<path>");
		...
		public bool ProcessArgument(string argument)
		{
			...
			{
                filePathText = match.Groups["fileName"].Value;
                return true;
			}
