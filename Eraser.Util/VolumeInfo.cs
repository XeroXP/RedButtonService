...
namespace Eraser.Util
{
	...
	public class VolumeStream : FileStream
	{
		...
		private void LockVolume()
		{
            try
            {
                var processInfos = LockManager.GetLockingProcessInfos(Volume.MountPoints.Select(mp => mp.FullName).ToArray(), LockManagerFeatures.CheckDirectories | LockManagerFeatures.UseLowLevelApi);
			...
			{
				NativeMethods.DeviceIoControl(SafeFileHandle,
						NativeMethods.FSCTL_DISMOUNT_VOLUME, IntPtr.Zero, 0, IntPtr.Zero,
